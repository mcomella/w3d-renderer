/**
 * The palette that assets index into. This palette is taken from
 * https://github.com/fabiensanglard/Chocolate-Wolfenstein-3D/blob/master/wolfpal.inc
 * as the original palette is linked into the source code and it's inconvenient to access.
 */
...
function RGB(r, g, b) {
    function sixBitTo8Bit(v) {
        // via https://github.com/fabiensanglard/Chocolate-Wolfenstein-3D/blob/eaade7d5c1887a144c6468d4777ef9f8841203ab/id_vl.cpp#L48
        // I guess for some reason these are 6-bit, not 8-bit, so we're converting to 8-bit.
        return v * 255 / 63;
    }
    return {r: sixBitTo8Bit(r), g: sixBitTo8Bit(g), b: sixBitTo8Bit(b)};
}
